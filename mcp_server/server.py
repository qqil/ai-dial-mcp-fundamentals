...
async def search_user(search_request: UserSearchRequest) -> str:
    """Search users by parameters: name, surname, email, gender"""
    users = await user_client.search_users(
...
async def add_user(user_create_model: UserCreate) -> str:
    ...
    result = await user_client.add_user(user_create_model)
    return result

@mcp.tool()
async def update_user(user_id: int, user_update_model: UserUpdate) -> str:
    """Update user by id with provided data"""
    result = await user_client.update_user(user_id, user_update_model)
    return result
...
@mcp.prompt()
def guidelines_for_formulating_search_queries() -> str:
...
@mcp.prompt()
def guidelines_for_creating_user_profiles() -> str:
